...
const getClient = () => {
    ...
    if (!key || key === "your-gemini-api-key") {
        throw new ApiError(503, "Gemini API key is not set");
    }
    if (!client) {
...
const extractJson = (text) => {
    ...
    if (start === -1) throw new ApiError(502, "Ai returned unexpected response");
...
const runPrompt = async (prompt) => {
    try {
        ...
        throw new ApiError(502, "AI service is temporary unavailable");
    }
};
...
No markdown, no commentary.`;
    ...
    if (!Array.isArray(json)) throw new ApiError(502, "AI did not return task list");
...
Respond ONLY with JSON: {
    ...
    "completed": string[] (key done itemss),
