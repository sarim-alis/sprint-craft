...
const deleteBoard = asyncHandler(async (req, res) => {
    if (req.board.role !== "owner")
        throw ApiError.forbidden("Only owner can delete this board");
...
const addMember = asyncHandler(async (req, res) => {
    ...
    await logActivity({
        ...
        message: `${req.user.name} added ${user.name} to board`,
...
const removeMember = asyncHandler(async (req, res) => {
    ...
    if (userId === req.board.owner_id)
        throw ApiError.badRequest("Cannot remove board owber");
